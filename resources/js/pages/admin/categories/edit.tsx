import {
    ...
    type FieldsConfig,
} from '@/components/admin/certificate-field-editor';
...
interface Props {
    ...
    event: Event & { is_lap_based?: boolean };
    event_is_lap_based?: boolean;
}
...
const sections: { id: SectionId; label: string; icon: typeof Settings2 }[] = [
    ...
    { id: 'lap-stats', label: 'Lap Stats', icon: Timer },
...
export default function CategoryEdit({ category, event_is_lap_based }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        {
            title: category.event?.title ?? 'Event',
            href: `/admin/events/${category.event?.slug}`,
        },
        {
            title: category.name,
    ...
    const categoryForm = useForm<{
        ...
        start_time: string;
        cut_off_time: string;
        lap_stats_config?: LapStatsConfig;
    }>({
        name: category.name,
        slug: category.slug,
        ...
        total_elevation_gain:
            category.total_elevation_gain ?? ('' as string | number),
        start_time: category.start_time
            ? new Date(category.start_time).toISOString().slice(0, 16)
            : '',
        cut_off_time: category.cut_off_time
            ? new Date(category.cut_off_time).toISOString().slice(0, 16)
            : '',
        lap_stats_config: category.lap_stats_config ?? undefined,
    });

    // Lap Stats form - only show if event is lap-based
    const defaultLapStats: LapStatsConfig = {
    ...
    const lapForm = useForm<LapStatsConfig>({
        total_laps_field:
            category.lap_stats_config?.total_laps_field ??
            defaultLapStats.total_laps_field,
        best_lap_field:
            category.lap_stats_config?.best_lap_field ??
            defaultLapStats.best_lap_field,
        avg_lap_field:
            category.lap_stats_config?.avg_lap_field ??
            defaultLapStats.avg_lap_field,
        current_cp_field:
            category.lap_stats_config?.current_cp_field ??
            defaultLapStats.current_cp_field,
        cp_time_field:
            category.lap_stats_config?.cp_time_field ??
            defaultLapStats.cp_time_field,
        segment_field:
            category.lap_stats_config?.segment_field ??
            defaultLapStats.segment_field,
    });

    const certForm = useForm<{
        template: File | null;
        enabled: boolean;
        fields_config: FieldsConfig | null | object;
    }>({
        template: null,
    ...
    const handleLapStatsSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        categoryForm.setData('lap_stats_config', lapForm.data as never);
        categoryForm.put(`/admin/categories/${category.slug}`);
    };
        ...
        checkpointForm.post(`/admin/categories/${category.slug}/checkpoints`, {
            onSuccess: () => {
                ...
                checkpointForm.setData(
                    'order_index',
                    (category.checkpoints?.length ?? 0) + 2,
                );
            },
        });
    };
    ...
    const handleUpdateCheckpoint = (e: React.FormEvent) => {
        ...
        checkpointForm.put(`/admin/checkpoints/${editingCheckpoint.id}`, {
            onSuccess: () => {
                ...
                checkpointForm.setData(
                    'order_index',
                    (category.checkpoints?.length ?? 0) + 1,
                );
            },
        });
    };
            ...
            <div className="flex h-full flex-1 flex-col p-4 md:p-6">
                ...
                <div className="flex flex-1 gap-6">
                    ...
                    <div className="flex-1 rounded-xl border bg-card p-6">
                                ...
                                >
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            type="url"
                                            value={
                                                categoryForm.data.endpoint_url
                                            }
                                            onChange={(e) =>
                                                categoryForm.setData(
                                                    'endpoint_url',
                                    ...
                                    <div className="grid gap-4 sm:grid-cols-2">
                                        ...
                                        <div className="space-y-2">
                                            <Label htmlFor="total_elevation_gain">
                                    ...
                                    <div className="grid gap-4 sm:grid-cols-2">
                                        <div className="space-y-2">
                                            ...
                                            <p className="text-xs text-muted-foreground">
                                                Race start time for cut-off
                                                calculation
                                            </p>
                                        </div>
                                        <div className="space-y-2">
                                            <Label htmlFor="cut_off_time">
                                                Cut Off Time
                                            </Label>
                                            <Input
                                                id="cut_off_time"
                                            ...
                                            />
                                            <p className="text-xs text-muted-foreground">
                                                Participants finishing after
                                                this time will be marked as COT
                                            </p>
                                        </div>
                                    </div>
                                    <div className="pt-4">
                                ...
                                <div className="space-y-3">
                                        ...
                                        <div key={cp.id}>
                                            {editingCheckpoint?.id === cp.id ? (
                                                <form
                                                    ...
                                                    className="space-y-4 rounded-lg border-2 border-primary/50 bg-muted/30 p-4"
                                                >
                                    ...
                                    {showNewCheckpoint && (
                                        <form
                                            ...
                                            className="space-y-4 rounded-lg border-2 border-dashed border-primary/30 bg-muted/20 p-4"
                                        >
                                            <div className="text-sm font-medium text-primary">
                                                New Checkpoint
                                            </div>
                        ...
                        {/* Lap Stats Section */}
                        {activeSection === 'lap-stats' && (
                            <div className="space-y-6">
                                <div>
                                    <h2 className="text-lg font-semibold">
                                        Lap Statistics Configuration
                                    </h2>
                                    <p className="text-sm text-muted-foreground">
                                        Map API response fields for lap-based
                                        event statistics
                                    </p>
                                </div>
                                <Separator />
                                ...
                                {event_is_lap_based ? (
                                    <form
                                        onSubmit={handleLapStatsSubmit}
                                        className="space-y-4"
                                    >
                                        <div className="grid gap-4 sm:grid-cols-2">
                                            <div className="space-y-2">
                                                <Label htmlFor="total_laps_field">
                                                    Total Laps Field
                                                </Label>
                                                <Input
                                                    id="total_laps_field"
                                                ...
                                                />
                                                <p className="text-xs text-muted-foreground">
                                                    Field name for total
                                                    completed laps
                                                </p>
                                            </div>
                                            <div className="space-y-2">
                                                <Label htmlFor="best_lap_field">
                                                    Best Lap Field
                                                </Label>
                                                <Input
                                                    id="best_lap_field"
                                                    ...
                                                    placeholder="e.g. BestLap"
                                                />
                                                <p className="text-xs text-muted-foreground">
                                                    Field name for best lap time
                                                </p>
                                            </div>
                                            <div className="space-y-2">
                                                <Label htmlFor="avg_lap_field">
                                                    Avg Lap Field
                                                </Label>
                                                <Input
                                                    id="avg_lap_field"
                                                    ...
                                                    placeholder="e.g. Avg.Lap"
                                                />
                                                <p className="text-xs text-muted-foreground">
                                                    Field name for average lap
                                                    time
                                                </p>
                                            </div>
                                            <div className="space-y-2">
                                                <Label htmlFor="current_cp_field">
                                                    Current CP Field
                                                </Label>
                                                <Input
                                                    id="current_cp_field"
                                        ...
                                        <div className="pt-4">
                                            ...
                                            >
                                                {categoryForm.processing
                                                    ? 'Saving...'
                                                    : 'Save Lap Stats Config'}
                                            </Button>
                                        </div>
                                    </form>
                                ) : (
                                    <div className="rounded-lg border-2 border-dashed p-8 text-center">
                                        <Timer className="mx-auto mb-4 h-12 w-12 text-muted-foreground/30" />
                                        <p className="text-muted-foreground">
                                            This category belongs to a
                                            non-lap-based event.
                                        </p>
                                        <p className="mt-2 text-sm text-muted-foreground">
                                            Enable "Lap-based event" in the
                                            event settings to configure lap
                                            stats.
                                        </p>
                                    </div>
                                )}
                            ...
                            <div className="space-y-6">
                                ...
                                >
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        />
                                        <p className="text-xs text-muted-foreground">
                                            Upload a GPX file to display
                                            elevation profile on the results
                                            page.
                                        </p>
                                    </div>
                        ...
                        {activeSection === 'certificate' && (
                            <div className="space-y-6">
                                <div>
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        Configure finisher certificate
                                        generation
                                    </p>
                                </div>
                                <Separator />
